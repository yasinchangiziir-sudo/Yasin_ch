...
def log_action(user_id, action, detail=""):
    ...
    if len(db["logs"]) > 500:   # نگه‌داری فقط ۵۰۰ لاگ آخر
        db["logs"] = db["logs"][-500:]
    save_db()
...
def register_user(user_id: str, username: str, first_name: str):
    if user_id not in db["users"]:
        db["users"][user_id] = {"username": username, "first_name": first_name,
...
def is_spam(user_id: str) -> bool:
    ...
    if len(user_last_messages[user_id]) >= 5:   # ۵ پیام در ۳ ثانیه = اسپم
        return True
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/mute - سکوت کاربر (در گروه، ادمین‌ها)\n"
        "/poll - نظرسنجی\n"
        "/remind - یادآوری\n"
        "امتیاز - تاپ - تاس - دارت - منو - قرعه‌کشی\n"
        "یادداشت: متن - یادداشت‌ها"
    )
...
async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if cmd == "stats":
        ...
        await update.message.reply_text(f"✅ ارسال شد به {ok} کاربر")
...
async def mute_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if member.status not in ["administrator", "creator"] and user.id != OWNER_ID:
        await update.message.reply_text("❌ فقط ادمین‌ها می‌تونن mute کنن.")
        return
    if not context.args or len(context.args) < 2:
        await update.message.reply_text("استفاده: /mute user_id دقیقه")
        return
    target_id = context.args[0]
    try:
        minutes = int(context.args[1])
    except:
        await update.message.reply_text("مدت زمان نامعتبر.")
        return
    target = get_user(target_id)
    if not target:
        await update.message.reply_text("کاربر تو ربات ثبت‌نام نکرده.")
        return
...
async def poll_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not text or "|" not in text:
        await update.message.reply_text("فرمت: /poll سوال | گزینه۱, گزینه۲,...")
        return
...
async def remind(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        await update.message.reply_text("استفاده: /remind 10m پیام\nمثال: /remind 30s سلام")
        return
    try:
        delay_str = context.args[0]
        message = " ".join(context.args[1:])
        if not message:
            await update.message.reply_text("متن یادآوری رو بنویس.")
            return
        ...
        if unit == 's': seconds = amount
            ...
            await update.message.reply_text("واحد نامعتبر. s/m/h")
            return
        await update.message.reply_text(f"⏰ یادآوری تنظیم شد برای {amount} {unit} دیگه.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "سازنده":
        ...
        notes = u.get("notes", [])
        if notes:
            await msg.reply_text("📒 یادداشت‌های تو:\n" + "\n".join(f"• {n}" for n in notes))
        else:
        ...
        try:
            admins = [m.user.id async for m in context.bot.get_chat_members(chat.id) if not m.user.is_bot]
            winner = random.choice(admins)
            await msg.reply_text(f"🎉 برنده: <a href='tg://user?id={winner}'>{winner}</a>", parse_mode="HTML")
        except:
            await msg.reply_text("خطا در قرعه‌کشی.")
    elif msg.photo:
